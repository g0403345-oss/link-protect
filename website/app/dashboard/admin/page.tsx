        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: '#52535a' }}>All {guilds.length} servers the bot is in</p>
            </div>
          ...
          <div style={{ position: 'relative', maxWidth: 360, marginBottom: 20 }}>
            ...
            <input type="text" placeholder="Search by server ID..." value={search} onChange={(e) => setSearch(e.target.value)}
          ...
          {loading && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 8 }}>
              {Array.from({ length: 12 }).map((_, i) => (
                <div key={i} style={{ height: 64, background: '#18181b', border: '1px solid #2e2e36', borderRadius: 10, animation: 'pulse 1.5s ease-in-out infinite' }} />
              ))}
            </div>
          )}
          ...
          {!loading && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 8 }}>
              {filtered.map((guildId, i) => (
                <motion.div key={guildId} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: Math.min(i * 0.02, 0.4) }}>
